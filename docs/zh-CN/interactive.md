...
`WidgetViewer` 会在笔记本保存格中绘制 `econ-viz` 图形，只要控制项一变动就重新绘制。从 `v1.4.0` 起，每个参数都同时有**滑块**和**数字输入框**，用户可以拖动培养直觉，也可以输入精确数值，方便教学与演示。
...
随包附带的 Playground 笔记本已经照这个流程设计，若环境中已经有 `econ-viz 1.4.0`，就不会重新安装。
