...
`WidgetViewer` 會在筆記本儲存格中繪製 `econ-viz` 圖形，只要控制項一變動就重新繪製。每個參數都同時有**滑桿**和**數字輸入框**，使用者可以拖曳培養直覺，也可以輸入精確數值，方便教學與示範。
...
隨套件附上的 Playground 筆記本已經照這個流程設計，若環境中已經裝好 `econ-viz`，就不會重新安裝。
